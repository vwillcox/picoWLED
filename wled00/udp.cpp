  ...
  switch (type) {
    ...
    case 2: //ArtNet
    {
      ...
      for (size_t currentPacket = 0; currentPacket < packetCount; currentPacket++) {
        ...
        ddpUdp.write((uint8_t)(sequenceNumber & 0xFF)); // sequence number. 1..255
        ddpUdp.write((uint8_t)0x00); // physical - more an FYI, not really used for anything. 0..3
        ddpUdp.write((uint8_t)((currentPacket) & 0xFF)); // Universe LSB. 1 full packet == 1 full universe, so just use current packet number.
        ddpUdp.write((uint8_t)0x00); // Universe MSB, unused.
        ddpUdp.write((uint8_t)(0xFF & (packetSize >> 8))); // 16-bit length of channel data, MSB
        ddpUdp.write((uint8_t)(0xFF & (packetSize     ))); // 16-bit length of channel data, LSB
